...
class Config:
    ...
    LEARNING_RATE = 0.001  # Increased for better training
    ...
    # Paths
    DATA_PATH = "dataset"
    MODEL_SAVE_PATH = "models/simplified_model"
...
def process_dataset():
    ...
    for file_path, label in tqdm(zip(train_files, train_labels), desc="Processing training files", total=len(train_files)):
        try:
...
def build_simple_cnn_model(input_shape, num_classes):
    """Build a simpler CNN model that works with older TensorFlow versions"""
    
    # Use Sequential API with error handling
    try:
        ...
        # For much older TensorFlow versions, use a different approach
...
def train_model(X_train, y_train, X_test, y_test, class_names):
    """Train the model with a simpler approach"""
    print("\nTraining model...")
    ...
    model = build_simple_cnn_model(input_shape, len(class_names))
    ...
    # Compile model
    optimizer = tf.keras.optimizers.Adam(lr=Config.LEARNING_RATE)  # Use 'lr' instead of 'learning_rate' for older TF
    ...
    try:
        ...
        es_callback = tf.keras.callbacks.EarlyStopping(
            monitor='val_acc',  # older TF uses 'val_acc' instead of 'val_accuracy'
            patience=Config.EARLY_STOPPING_PATIENCE
        )
        callbacks_list.append(es_callback)
    ...
    # Try both naming conventions for metrics
    ...
    return model, y_test_encoded, y_pred, test_acc

def main():
    """Main function"""
    print("Starting Simplified Lao Instrument Classifier Training...")
